...
ParticleSystemFire1.prototype = {
	...
	draw: function(ctx,section) {
		ctx.inject(this._doDraw,this,section);
	},
	_doDraw: function(self,section) { // this==ctx
		if(!self.tex) return;
		var	pMatrix = this.pMatrix,
			animTime = this.pathTime,
			mvMatrix = section.getMvMatrix(animTime),
			mvp = mat4_multiply(mat4_multiply(pMatrix,mvMatrix),self.mvMatrix),
			colour = section == modMenu.active? [1,0,0,0.8]: null;
		if(self.debugCtx)
		...
		gl.uniform4fv(program.uColour,colour||OPAQUE);
